...
func newTestIPReputationStoreWithDB(t *testing.T, cfg config.IPReputationConfig, db *sqlstore.Store, log *slog.Logger) *ipReputationStore {
	...
	store := newIPReputationStore(cfg, repository, log)
	ipReputationTestStates.Store(store, state)
	t.Cleanup(func() { ipReputationTestStates.Delete(store) })
	return store
}
...
func TestRejectedIPCacheExpiresEntries(t *testing.T) {
	...
	if !cache.add(context.Background(), addr, connectionDNSResult{}) {
...
func TestRejectedIPCacheCanonicalizesIPv6Zones(t *testing.T) {
	...
	if !cache.add(context.Background(), zoned, connectionDNSResult{}) {
...
func TestRejectedIPCacheHonorsAllowlist(t *testing.T) {
	...
	for _, value := range []string{"192.0.2.25", "198.51.100.25", "2001:db8::1"} {
		if cache.add(context.Background(), netip.MustParseAddr(value), connectionDNSResult{}) {
			t.Errorf("allowlisted address %s was added", value)
		}
	}
	if !cache.add(context.Background(), netip.MustParseAddr("203.0.113.25"), connectionDNSResult{}) {
...
func TestRejectedIPCleanupEvictsOldestShortBlock(t *testing.T) {
	...
	cache.add(context.Background(), first, connectionDNSResult{})
...
func TestRejectedIPUnknownLookupDoesNotWaitForWriter(t *testing.T) {
	...
	go func() {
		writerDone <- ipTestDatabase(cache).WithTx(context.Background(), nil, func(tx *sql.Tx) error {
...
func TestRejectedIPCacheHonorsForwardConfirmedDomainAllowlist(t *testing.T) {
	...
	if cache.add(context.Background(), addr, dns) {
...
func TestDomainAllowlistFailsOpenOnUnavailableDNSResult(t *testing.T) {
	...
	if !cache.add(context.Background(), netip.MustParseAddr("192.0.2.25"), connectionDNSResult{status: message.ReverseDNSLookupFailed}) {
...
func TestRejectedIPCachePersistsReputation(t *testing.T) {
	...
	t.Cleanup(func() { _ = db.Close() })
	cache := newTestIPReputationStoreWithDB(t, policy, db, nil)
	setIPTestClock(cache, func() time.Time { return now })
	addr := netip.MustParseAddr("192.0.2.43")
	cache.add(context.Background(), addr, connectionDNSResult{})
	now = now.Add(2 * time.Minute)
	cache.add(context.Background(), addr, connectionDNSResult{})
	reloaded := newTestIPReputationStoreWithDB(t, policy, db, nil)
...
func TestIPReputationRetainsDistinctStrikesAtSameMillisecond(t *testing.T) {
	...
	setIPTestClock(store, func() time.Time { return now })
...
func TestIPReputationConcurrentStrikesAreAtomic(t *testing.T) {
	...
	setIPTestClock(store, func() time.Time { return time.Date(2026, 9, 10, 12, 0, 0, 0, time.UTC) })
	addr := netip.MustParseAddr("192.0.2.101")
	var wait sync.WaitGroup
	for range strikes {
		wait.Add(1)
		go func() { defer wait.Done(); store.add(context.Background(), addr, connectionDNSResult{}) }()
	}
	wait.Wait()
	block, found := store.lookup(context.Background(), addr)
...
func TestIPReputationLookupIndexes(t *testing.T) {
	...
	for _, test := range []struct{ query, index string }{
		...
		rows, err := ipTestDatabase(store).Query(context.Background(), "EXPLAIN QUERY PLAN "+test.query)
		if err != nil {
			t.Fatal(err)
		}
		var plan string
